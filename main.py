...
restart_cooldown = {}  # Track restart attempts
RESTART_COOLDOWN_MINUTES = 10  # Nuclear restart cooldown
    ...
    def trigger_nuclear_restart(self, chat_id):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                os.remove(self.restart_flag_file)  # Clean up flag
...
• Buffer: {len(semi_fetch_buffer)} posts
• Uptime: 0 minutes (fresh start)

🔄 First RSS fetch in progress...
"""
...
2. **Manual Restart** - [Click here to restart manually]({manual_link}) (opens Render dashboard)
   ...
   - Wait 30-45 seconds
   - Bot will auto-resume

📞 **Need Help?** Check [Render Status](https://status.render.com) or contact support.
"""
...
def home():
    ...
    <html>
        <body>
            ...
            <p>Buffer: {buffer_size} posts</p>
            <p>Uptime: {hours}h {minutes}m {seconds}s</p>
            <p>Links sent: {restart_manager.get_sent_link_count():,}</p>
...
def restart_command(message):
    ...
    if user_id in restart_cooldown:
        ...
        if time_since_last < RESTART_COOLDOWN_MINUTES * 60:
            minutes_left = int((RESTART_COOLDOWN_MINUTES * 60 - time_since_last) / 60)
            bot.reply_to(message, 
...
def stop_command(message):
    """Legacy stop command - kept for compatibility"""
    if str(message.chat.id) == str(USER_CHAT_ID):
        bot.reply_to(message, 
            "⚠️ Use `/restart` for nuclear restart instead.\n"
            "For emergency stop, use Render dashboard."
        )
    else:
...
def initialize_bot():
    ...
    try:
        ...
        # Send startup message (only if not a restart)
        if not restart_data:
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting THOT RSS Bot v2.0 (Nuclear Restart Enabled)")
