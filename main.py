...
USER_CHAT_ID = "1168907278"   # stored as string for Supabase consistency
BATCH_SIZE = 5
BATCH_SEND_INTERVAL = 600
PULSE_DELAY = 7
...
# ------------------- SAFE SEND (THE FIX YOU NEEDED) -------------------
def safe_send(chat_id, msg, markdown=False):
    for attempt in range(5):
        try:
            ...
            print(f"[SEND ERROR] Attempt {attempt+1}/5 failed: {e}")
            time.sleep(5)
    print("[SEND ERROR] All retry attempts failed.")
    return False
...
def send_batch():
    while True:
        if semi_fetch_buffer:
            ...
            for post in batch:
                msg = f"📘 **THOT SIGNAL**\n**{post['title']}**\n{post['link']}"
                safe_send(USER_CHAT_ID, msg, markdown=True)
                mark_sent(USER_CHAT_ID, post["link"])
                time.sleep(PULSE_DELAY)

            safe_send(USER_CHAT_ID, "📢 New updates are coming!")
...
def status_loop():
    while True:
        ...
        safe_send(USER_CHAT_ID, "📘 THOT STATUS:\nScanning networks.\nProcessing signals.\nPipelines clean.")

# ------------------- AUTO START -------------------
def send_start_message():
    safe_send(USER_CHAT_ID, "📘 THOT is online.\nCore systems calibrated.\nSignal flow initiated.")
